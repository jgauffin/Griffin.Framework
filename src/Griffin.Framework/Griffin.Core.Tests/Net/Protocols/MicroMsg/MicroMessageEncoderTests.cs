...
namespace Griffin.Core.Tests.Net.Protocols.MicroMsg
{
    ...
    {
        [Fact]
        public async Task dont_close_our_internal_stream_as_we_reuse_it()
        {
            var serializer = new StringSerializer();
            var slice = new StandAloneBuffer(new byte[65535], 0, 65535);
            var msg = "Hello world";
            var channel = Substitute.For<IBinaryChannel>();

            var sut = new MicroMessageEncoder(serializer, slice);
            await sut.EncodeAsync(msg, channel);
            sut.Clear();
        ...
        [Fact]
        public async Task can_clear_even_without_shit_being_used()
        {
            var serializer = new StringSerializer();
            var slice = new StandAloneBuffer(new byte[65535], 0, 65535);
            var msg = "Hello world";
        ...
        [Fact]
        public async Task close_external_Stream_as_we_take_over_ownership_from_the_dev()
        {
            var serializer = new StringSerializer();
            var slice = new StandAloneBuffer(new byte[65535], 0, 65535);
        ...
        [Fact]
        public async Task write_a_complete_string_message_directly()
        {
        ...
        [Fact]
        public async Task partial_send__continue_sending_rest_of_the_buffer_before_doing_anything_else()
        {
            var contentType = "text/plain;type=System.String";
            var serializer = new StringSerializer();
            var slice = new StandAloneBuffer(new byte[65535], 0, 65535);
        ...
        [Fact]
        public async Task too_small_buffer_requires_multiple_sends()
        {
            var contentType = "text/plain;type=System.String";
            var serializer = new StringSerializer();
            var slice = new StandAloneBuffer(new byte[520], 0, 520);
        ...
        [Fact]
        public async Task reset_after_each_successful_message()
        {
            var contentType = "text/plain;type=System.String";
            var serializer = new StringSerializer();
            var slice = new StandAloneBuffer(new byte[520], 0, 520);
